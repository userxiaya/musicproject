...
import eeui.android.audio.event.tool;
...
public class MusicService {
    ...
    private class PlayAsyncTask extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... strings) {
            isPreview = false;
            Boolean urlCanUse = tool.isConnect(strings[0]);
            if(urlCanUse) {
                setUrl(strings[0]);
                setListener();
                statTimer();
    ...
    public boolean playNext(String url) {
        ...
        Boolean urlCanUse = tool.isConnect(url);
        if(urlCanUse == false){
            eventPost(url, AudioEvent.STATE_ERROR);
            cancelTimer();
            eventPost(url, AudioEvent.STATE_COMPELETE);
    ...
    public void pause() {
        if (mPlayer != null) {
            ...
            if(backService!=null) {
                backService.updatePlayerState(R.drawable.ic_play);
            }
    ...
    public void start() {
        if (mPlayer != null) {
            ...
            BackService backService = BackService.getService();
            if(backService!=null) {
                backService.updatePlayerState(R.drawable.ic_pause);
            }
        }
    }
    ...
    public void nextSong() {
        eventPost(url, AudioEvent.STATE_NEXT_SONG);
    }
    public void lastSong() {
        eventPost(url, AudioEvent.STATE_LAST_SONG);
    }
    ...
    private void eventPost(String url, int state) {
        try {
            if (mPlayer == null || isPreview==false) {
